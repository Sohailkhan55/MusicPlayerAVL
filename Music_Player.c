...
//     if (balance > 1 && strcmp(curr->key.singer, (node->left)->key.singer) < 0)
//     {
//       ret_val = rightRotate(node);
//     }
//     else if (balance < -1 && strcmp(curr->key.singer, (node->right)->key.singer) > 0)
//     {
//       ret_val = leftRotate(node);
//     }
//     else if (balance > 1 && strcmp(curr->key.singer, (node->left)->key.singer) > 0) 
//     {
//       node->left = leftRotate(node->left);
//       ret_val = rightRotate(node);  
//     }
//     else if (balance < -1 && strcmp(curr->key.singer, (node->right)->key.singer) && ret_val==NULL) {
//       node->right = rightRotate(node->right);
//       ret_val = leftRotate(node);
//     }
//   }
...
// Node *deleteNode(Node *root, char* song)
// {
//   Node *result = root;
...
//   if (root != NULL)
//   {
//     if (strcmp(song, root->key.song) < 0)
//     {
//       root->left = deleteNode(root->left, song);
//     }
//     else if (strcmp(song, root->key.song) > 0)
//     {
...
//         root->right = deleteNode(root->right, temp->key.song);
//       }
//     }
...
//         if (strcmp(root->key.song, node->key.song) == 0 && root->right != NULL)
//             return findMinimum(root->right);
//         // if node does not have a right subtree
//         else if (strcmp(node->key.song, root->key.song) < 0){
//             succ = root;
//             root = root->left;
//         }
//         else if (strcmp(node->key.song, root->key.song) > 0)
...
//   {
//       // if right subtree exist
//       if (strcmp(root->key.song, node->key.song) == 0 && root->left != NULL)
...
void Print(Node *root, char singer1[], char singer2[])
{
  ...
  //  else if(strcmp(singer1,root->key.singer) <= 0 && strcmp(singer2,root->key.singer)>=0)
  //  {
  //   printf("Song: %s\n",root->key.song);
  //   printf("Singer: %s\n", root->key.singer);
  //   printf("Lyricist: %s\n", root->key.lyricist);
      ...
      if(t>=0)
      {
          switch(t)
          {
              ...
              case 2: PrintAsc(root);
                      break;
              case 3: PrintAsc(root);
                      PrintDes(root);
                      break;
              case 4:; char str1[100];
                      ...
                      printf("Successfully deleted songs with given singer\n");
